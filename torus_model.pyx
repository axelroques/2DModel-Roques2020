...
def Euler_method_for_ring_model(str NRN1, str NRN2, str NTWK, str RING, str STIM, float BIN=5e-3, \
    ...
    '''
    cdef dict simulation_parameters, params
    cdef object[:, :] M
    ...
    M_conn_exc, M_conn_inh, nb_exc_neighb, nb_inh_neighb = \
        np.load('data/conn_matrix_torus.npy', allow_pickle=True)
    ...
    for i_t in range(len(t)-1): 
        ...
        for i_z in range(len(Z)): 
            for i_x in range(len(X)): 
                ...
                # SNR = 10
                # # Calculate noise 
                # noise_fe_mean = fe_pure_exc/50
                # noise_fi_mean = 0.04
                # # Generate an sample of white noise
                # noise_fe = np.random.normal(0, noise_fe_mean)
                # noise_fi = np.random.normal(0, noise_fi_mean)
                # # Noise 'em up
                # fe_pure_exc += noise_fe
                # fi += noise_fi


                # Excitatory neighbours
                for i_exc in range(nb_exc_neighb):
                ...
                for i_inh in range(nb_inh_neighb):
                    # Delay in propagation due to limited axon conduction
    ...
    return simulation_parameters, t, X, Z, Fe_aff, Fe, Fi, np.abs((muVn-muV0)/muV0)
